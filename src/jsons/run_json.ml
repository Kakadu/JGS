  ...
  let rec loop i stream =
    if i > n then ()
      ...
      | None, Some (h, tl) ->
          ...
          loop (1 + i) tl
  in
  loop 1 @@ OCanren.(run q) query (fun q -> q#reify JGS_lib.JGS.HO.jtype_reify)
...
let () =
  ...
  let open JGS2 in
  let (module CT : JGS2.SAMPLE_CLASSTABLE), goal, name_of_id =
  ...
  let pp ppf x =
    ...
    in
    Format.fprintf ppf "%a" (JGS2.pp_jtyp_logic lookup) x
  in
