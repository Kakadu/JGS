...
let make_sample_ct () = (module JGS2.SampleCT : JGS2.SAMPLE_CLASSTABLE)
...
let make_classtable table =
  let ((module CT : JGS2.SAMPLE_CLASSTABLE) as ct) = make_sample_ct () in
    ...
    make_vars
      ...
      (fun lookup ->
        ...
        let rec on_typ : _ -> JGS.HO.jtype_injected = function
          | Class (name, args) -> (
              match id_of_name name with
              | cid -> JGS2.class_ (Std.nat cid) (Std.list on_arg args)
              | exception Not_found ->
                  failwiths "Can't find class name '%s'" name)
          | Interface (name, args) -> (
              match id_of_name name with
              | cid -> JGS2.interface (Std.nat cid) (Std.list on_arg args)
              | exception Not_found ->
                  failwiths "Can't find class name '%s'" name)
          | Var { id; _ } -> lookup id
          | t ->
        ...
        and on_arg : _ -> _ JGS.HO.targ_injected = function
          ...
          | Wildcard None -> JGS2.wildcard (Std.none ())
          | Wildcard (Some (kind, typ)) ->
              ...
              in
              JGS2.wildcard (Std.some !!(pol, on_typ typ))
          | t -> JGS2.type_ (on_typ t)
        in
